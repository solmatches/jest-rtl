...
test("토핑을 선택하지 않은 경우", async () => {
	...
	expect(ChocolateScoop).toHaveValue(2);
	...
	const scoopTotal = screen.getByRole("heading", { name: "Scoops: 4,000원" });
	expect(scoopTotal).toBeInTheDocument();

	const toppingTotal = screen.queryByRole("heading", {
		name: "Toppings: 0원",
	});
	expect(toppingTotal).not.toBeInTheDocument();
	...
	// 메뉴들이 잘 불러와졌는지 확인.
	const initChocolateScoop = screen.getByRole("spinbutton", {
		name: "Chocolate",
	});
